...
namespace ArrayVisualizer
{
...

  {
    ...
    protected override void DrawContent()
    {
      ...
      using (Graphics gr = Graphics.FromImage(this.Image))
      {
        ...
        for (int y = 0; y < ySize; y++)
          ...
          {
            string textToRender = ((double)this.data.GetValue(0, y, x)).ToString(this.formatter);
        ...
        for (int z = 0; z < zSize; z++)
          for (int x = 0; x < xSize; x++)
          {
            string textToRender = ((double)this.data.GetValue(z, 0, x)).ToString(this.formatter);
            SizeF textSize = gr.MeasureString(textToRender, this.RederFont);
        ...
        for (int z = 0; z < zSize; z++)
          for (int y = 0; y < ySize; y++)
          {
            string textToRender = ((double)this.data.GetValue(z, y, xSize - 1)).ToString(this.formatter);
            SizeF textSize = gr.MeasureString(textToRender, this.RederFont);
    ...
    {
      if (this.Data.Rank != 3)
        throw new ArrayTypeMismatchException("source array must be three dimensional.");
